<?php
/**
 * Rule for searching dependencies in frontend files
 *
...
class LayoutRule implements \Magento\TestFramework\Dependency\RuleInterface
{
    ...
    /**
     * Check frontend handles
     *
     * Ex.: <frontend><{name}>...</frontend>
     *
     * @param string $currentModule
     * @param string $file
     * @param string $contents
    ...
    protected function _caseLayoutHandle($currentModule, $file, &$contents)
    {
        ...
        $result = [];
        foreach ((array)$xml->xpath('/frontend/child::*') as $element) {
    ...
    /**
     * Check frontend handles parents
     *
    ...
    protected function _caseLayoutHandleParent($currentModule, $file, &$contents)
    {
        ...
        $result = [];
        foreach ((array)$xml->xpath('/frontend/child::*/@parent') as $element) {
    ...
    /**
     * Check frontend handles updates
     *
    ...
    /**
     * Check frontend handle dependency
     *
     * Return: array(
    ...
    /**
     * Check frontend block dependency
     *
     * Return: array(
     *  'modules'  // dependent modules
     *  'source'  // source text
