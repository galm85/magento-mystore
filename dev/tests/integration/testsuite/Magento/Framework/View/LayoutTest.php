        ...
        $merge->expects($this->once())
            ...
            ->willReturn(
                simplexml_load_string(
                    '<frontend><container name="container1"></container></frontend>',
                    Element::class
                )
            );
        $layout->expects($this->once())->method('getUpdate')->willReturn($merge);
        $this->assertEmpty($layout->getXpath('/frontend/container[@name="container1"]'));
        $layout->generateXml();
        $this->assertNotEmpty($layout->getXpath('/frontend/container[@name="container1"]'));
    }
