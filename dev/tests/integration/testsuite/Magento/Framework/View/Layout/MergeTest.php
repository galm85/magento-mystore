...
class MergeTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testLoadCache()
    {
        $cacheValue = [
            "pageLayout" => "1column",
            "frontend"     => self::FIXTURE_LAYOUT_XML
        ];
    ...
    {
        $errorString = "Theme frontend update file '" . __DIR__ . "/_mergeFiles/frontend/file_wrong.xml' is not valid.";
    ...
    public function testLoadWithInvalidArgumentThrowsException()
    {
        ...
        $this->expectExceptionMessage('Invalid frontend update handle');

        $this->_model->load(123);
    }

    /**
     * Test loading invalid frontend
     *
     */
    public function testLoadWithInvalidLayout()
    {
        ...
        $this->_logger->expects($this->once())->method('info')
            ->with(
                'Cache file with merged frontend: ' . $cacheId . ' and handles default' . ': ' . array_shift($messages)
            );
    ...
    public function testLayoutUpdateFileIsNotValid()
    {
        ...
        $this->expectExceptionMessageMatches('/_mergeFiles\\/frontend\\/file_wrong\\.xml\\\' is not valid/');
