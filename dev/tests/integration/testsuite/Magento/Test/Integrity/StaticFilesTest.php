...
class StaticFilesTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testReferencesFromLayoutFiles($layoutFile, $area, $themePath, $fileId)
    {
        ...
        $this->assertNotEmpty(
            $this->getStaticFile($area, $themePath, 'en_US', $filePath, $module),
            "Unable to locate '{$fileId}' reference from frontend XML in {$layoutFile}"
        );
    }
    ...
    public function referencesFromLayoutFilesDataProvider()
    {
        ...
        foreach ($files as $metaInfo) {
            ...
            if (!is_string($file)) {
                $this->addWarning(
                    'Wrong frontend file configuration provided. The `file` meta info must be the type of string'
                );
                continue;
            }
    ...
    /**
     * Collect view file declarations in frontend XML-files
     *
     * @param string $file
     * @return array
     */
