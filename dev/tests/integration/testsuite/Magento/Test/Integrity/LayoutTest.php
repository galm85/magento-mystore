...
class LayoutTest extends \PHPUnit\Framework\TestCase
{
    ...
    /**
     * Composes full frontend xml for designated parameters
     *
     ...
     * For example, a module frontend file is overlapped by theme frontend, which has mistakes.
    ...
    public function testPageTypesDeclaration()
    {
        ...
        $invoker(
            /**
             * Check whether page types are declared only in frontend update files allowed for it - base ones
             */
            function (\Magento\Framework\View\File $layout) {
                ...
                $this->assertEmpty(
                    ...
                    "Theme frontend update '" . $layout->getFilename() . "' contains page type declaration(s)"
                );
            },
    ...
    public function pageTypesDeclarationDataProvider()
    {
        ...
        $themeUpdates = \Magento\TestFramework\Helper\Bootstrap::getObjectManager()
            ->create(\Magento\Framework\View\File\Collector\ThemeModular::class, ['subDir' => 'frontend']);
        ...
        $themeUpdatesOverride = \Magento\TestFramework\Helper\Bootstrap::getObjectManager()
            ->create(
                ...
                ['subDir' => 'frontend/override/theme']
            );
            ...
            function ($themeFile, $theme) {
                ...
                $this->assertNotNull(
                    ...
                    sprintf(
                        ...
                        'its frontend file is supposed to be overridden by file "%s".',
    ...
    protected static function _getCachedFiles(
        ...
        if (!isset(self::$_cachedFiles[$cacheKey])) {
            ...
            $fileList = \Magento\TestFramework\Helper\Bootstrap::getObjectManager()
                ->create($sourceClass, ['subDir' => 'frontend'])->getFiles($theme, '*.xml');
            $files = [];
